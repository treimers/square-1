...
public class Layer {
	...
	static {
		...
		for (int i = 0; i < HalfLayer.list.length; i++) {
			for (int j = 0; j < HalfLayer.list.length; j++) {
				...
				if (HalfLayer.list[i].getNPieces() + HalfLayer.list[j].getNPieces() <= 10) {
					list[lll++] = new Layer(HalfLayer.list[i], HalfLayer.list[j]);
				}
			}
		}
	}
	...
	private int turnt, turnb; // Amount to turn (clockwise, counter-cw) till next unblocked seam
	private int nPieces; // total number of pieces in the layer
	private boolean turnParityOdd, turnParityOddb; // permutation parity of a (cw/ccw) turn
