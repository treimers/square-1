...
public class Sq1Shape {
		...
		for (int i = 0; i < Layer.list.length; i++) {
			for (int j = 0; j < Layer.list.length; j++) {
				...
				if (Layer.list[i].getNPieces() + Layer.list[j].getNPieces() == 16) {
					list[nShape++] = new Sq1Shape(Layer.list[i], Layer.list[j], true);
					list[nShape++] = new Sq1Shape(Layer.list[i], Layer.list[j], false);
				}
			}
		}
	}
	...
	private int tpieces[] = new int[4]; // Resulting shape pattern after move or after reflection
	private boolean tparity[] = new boolean[4]; // Resulting parity after move or after reflection
	...
	Sq1Shape(Layer l1, Layer l2, boolean p) {
		...
		tpieces[2] = (l1.getH1().getPieces() << 18) + (l2.getH1().getPieces() << 12)
		...
		tparity[2] = parityOdd ^ ((l1.getH2().getNPieces() & l2.getH1().getNPieces() & 1) != 0);
		tparity[3] = parityOdd;
	}
