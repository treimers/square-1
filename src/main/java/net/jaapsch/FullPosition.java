...
public class FullPosition {
	...
	void print() {
		for (int i = 0; i < 24; i++) {
			System.out.print("ABCDEFGH12345678".charAt(pos[i]));
			// skip second half of a corner piece
			if (pos[i] < 8)
				i++;
		}
		System.out.println("/ -".charAt(middle + 1));
	}
	...
	void random() {
		...
		do {
			...
			for (int i = 0; i < 16; i++)
				tmp[i] = i;
			for (int i = 0; i < 16; i++) {
				int j = (int) Math.floor(Math.random() * (16 - i));
				int k = tmp[i];
				tmp[i] = tmp[i + j];
				tmp[i + j] = k;
			}
			...
			// test legal and twistable (i.e. no corner over seam or split over layers)
	...
	int getEdgeColouring(int cl) {
		final int EDGE_clp[][] = { { 8, 9, 10, 11 }, { 8, 9, 13, 14 }, { 15, 14, 10, 9 } };
		int c = 0;
	...
	int getCornerColouring(int cl) {
		final int CORNER_clp[][] = { { 0, 1, 2, 3 }, { 0, 1, 5, 6 }, { 7, 6, 2, 1 } };
		int c = 0;
		// set mask to bit for first edge
		int m = (cl != 2) ? 1 << 7 : 1;
		// for each corner
	...
	// parse input string; either position or move sequence that solves the position
	String parseInput(String inp) {
		// check characters
		int f = 0;
		...
		if (f == 1) {
			...
			for (int i = inp.length() - 1; i >= 0; i--) {
				...
				if (md == 0) { // parsing any move
					if (k == '/') {
						if (!doTwist())
							return "Twist is blocked by corner.";
						lu++;
						lw++;
					} else {
						bt = 0;
						vl = 1;
						md++;
						i++; // try this character again, assuming it starts bottom layer turn
					}
					...
					if (k == ')' && vl == 1) { // skip bracket
						...
						return "Bottom layer turn expected.";
					} else if (k == '-') { // parse minus sign
						bt = -bt;
						md++; // end of bottom layer turn
					} else if (k == ',') { // comma signals end of bottom layer turn
						i++;
						md++;
					} else
						return "Bottom layer turn expected.";
					...
					if (k >= '0' && k <= '9') { // store digit
						...
						return "Top layer turn expected.";
					} else if (k == '-') { // parse minus sign
						tt = -tt;
			...
			System.out.println("Input:" + inp + " [" + lw + "|" + lu + "]");
		} else {
			// position
			if (inp.length() != 16 && inp.length() != 17)
				return "Position should be 16 or 17 characters.";
			int j = 0;
			int pi[] = new int[24];
			// for each character
			for (int i = 0; i < 16; i++) {
