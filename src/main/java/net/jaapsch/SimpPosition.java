...
public class SimpPosition {
	...
	SimpPosition(ShapeTranTable stt0, ShpColTranTable scte0,
	...
	void set(FullPosition p, boolean turnMetric0) {
	...
	public String solve() {
		...
		do {
			// increase depth.
			ln++;
			// search only even/odd lengths if twist metric and care about middle layer
		...
		} while (search(sb, ln, 3) == 0);
		return sb.toString();
	}

	// Solve current position in exactly ln moves, last move was lm (0=top, 1=bot,
	// 2=twist)
	int search(StringBuffer sb, int ln, int lm) {
		int i, r = 0;
		...
		if (lm >= 2) { // last move was not top/bottom layer
			...
			do {
				...
				if (turnMetric || i < 6 || ln < 2) {
					...
					lastTurns[4] = i;
					// search deeper
					r += search(sb, turnMetric ? ln - 1 : ln, 0);
					// forget move
					moveLen--;
		...
		if (lm != 1) { // last move was not bottom layer
			...
			do {
				...
				lastTurns[5] = i;
				// search deeper
				r += search(sb, turnMetric ? ln - 1 : ln, 1);
				// forget move
				moveLen--;
				// quit if found solution
				if (r != 0)
	...
	void printsol(StringBuffer sb, long nodes) {
		...
		for (int i = 0; i < moveLen; i++) {
			...
			if (move == 0) { // twist move
				...
				tw++;
				lm = 2;
			} else if (move < 12) { // top layer move
